...
const Header = ({ gameData, formatDate }) => {
  ...
  return (
    <>
          ...
          <h2 className="text-2xl">
            ...
            {gameData.quote?.split(/\s+/).map((word, index) => (
              <span
                key={index}
                className={`
                  ${gameData.progress[index] ? "text-green-850" : "text-white"}
      ...
      <div className="hidden md:block bg-gray-900 text-white z-10 flex-shrink-0 rounded rounded-b-lg mt-3">
        <div className="p-4 flex items-center">
          ...
          <div className="flex-1 text-center">
            <button 
              className="text-sm underline mx-auto hover:cursor-pointer"
              onClick={() => setShowModal(true)}
            >
